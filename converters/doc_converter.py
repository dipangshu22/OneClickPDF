...
def _render_paragraph(para, hmap):
    ...
    # spacing — cap tightly to prevent content overflow onto extra pages
    try:
        sb = para.paragraph_format.space_before
        if sb and sb.pt > 0:
            ps.append(f'margin-top:{min(sb.pt, 6):.1f}pt')
    except Exception: pass
    try:
        sa = para.paragraph_format.space_after
        if sa and sa.pt > 0:
            ps.append(f'margin-bottom:{min(sa.pt, 4):.1f}pt')
    except Exception: pass
...
def _docx_to_html(input_path: Path, margins: dict) -> str:
    ...
    pw = margins['page_w']; ph = margins['page_h']
...
def _html_to_pdf(html: str, output_path: str, tmp_dir: Path, margins: dict):
    """Render HTML → PDF via xhtml2pdf (pure Python, no system deps)."""
...
def _count_docx_pages(doc) -> int:
    """
    Estimate page count from DOCX. Returns the number of explicit page breaks + 1.
    Always returns at least 1 (every document has at least one page).
    """
    try:
        from docx.oxml.ns import qn as _qn
        pb = doc.element.body.findall(f'.//{_qn("w:br")}')
        page_breaks = [b for b in pb if b.get(_qn('w:type')) == 'page']
        return len(page_breaks) + 1
    except Exception:
        return 1  # safe default


def _enforce_page_count(pdf_path: str, expected_pages: int):
    """Trim the PDF to exactly expected_pages if it exceeds that count."""
    reader = PdfReader(pdf_path)
    actual = len(reader.pages)
    if actual <= expected_pages:
        return
    logger.info(f"Trimming PDF from {actual} to {expected_pages} page(s).")
    writer = PdfWriter()
...
def _page_raw_size(page) -> int:
    """Return the raw content stream size of a PDF page (proxy for how much is on it)."""
    raw_size = 0
    if '/Contents' in page:
        c = page['/Contents']
        if hasattr(c, 'get_data'):
...
def _strip_blank_trailing_pages(pdf_path: str):
    """
    Remove trailing pages that are blank or contain only minor overflow content.
    A page is considered overflow if:
      - It is completely blank (no text, raw_size < 100), OR
      - It has very little text (< 80 chars) AND is much smaller than the first page
        (raw_size < 15% of page-1's raw_size), suggesting a few words spilled over.
    """
    reader = PdfReader(pdf_path)
    pages  = list(reader.pages)
    before = len(pages)
    ...
    # Measure the first page as a baseline
    first_text     = (pages[0].extract_text() or '').strip()
    first_raw_size = _page_raw_size(pages[0])
    overflow_raw_threshold = max(first_raw_size * 0.15, 200)  # 15% of p1, min 200 bytes

    while len(pages) > 1:
        last     = pages[-1]
        text     = (last.extract_text() or '').strip()
        raw_size = _page_raw_size(last)

        truly_blank  = (text == '' and raw_size < 100)
        is_overflow  = (len(text) < 80 and raw_size < overflow_raw_threshold)

        if truly_blank or is_overflow:
            pages.pop()
            logger.info(f"Stripped trailing page (text={len(text)} chars, "
                        f"raw={raw_size}b, threshold={overflow_raw_threshold:.0f}b).")
        else:
            break
...
def convert_docx_to_pdf(input_path: str, output_path: str) -> dict:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        try:
            ...
            expected_pages = _count_docx_pages(doc)
            ...
            logger.info("Step 3: HTML → PDF (xhtml2pdf)")
            _html_to_pdf(html, output_path, tmp, margins)

            logger.info("Step 4: Strip blank/overflow trailing pages")
            _strip_blank_trailing_pages(output_path)

            logger.info(f"Step 5: Enforcing page count = {expected_pages}")
            _enforce_page_count(output_path, expected_pages)
