...
def convert():
    ...
    pages_in_merged = len(PdfReader(str(merged_path)).pages)
    summary = f"{len(successful)} file(s) merged into one PDF ({pages_in_merged} page(s), {size_kb:.1f} KB)."
    if failed:
...
def translate_file():
    ...
    try:
        ...
        # ── Quick check: any extractable text? ────────────────────────────
        with fitz.open(str(input_path)) as _chk:
            total_pages = len(_chk)
            has_text    = any(
                _chk[i].get_text("text").strip()
                for i in range(min(3, total_pages))   # check first 3 pages only
            )
        if not has_text:
            return jsonify({
                'success': False,
        ...
        # Open source + a blank output document
        ...
        for page_num in range(total_pages):
            ...
            # Copy page into output doc (preserves size, images, vectors)
            out_doc.insert_pdf(src, from_page=page_num, to_page=page_num)
            page = out_doc[-1]

            # ── Collect text lines with bbox / style ──────────────────────
            ...
            # Free per-page data immediately
            del lines_info, text_dict
